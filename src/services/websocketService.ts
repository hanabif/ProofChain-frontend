...
class WebSocketService {
  ...
  private url =
    import.meta.env.VITE_WS_URL || "ws://127.0.0.1:8000/ws/transactions/";
  ...
  connect() {
    ...
    this.socket.onopen = () => {
      console.log(
        "%c[WS DEBUG] ✅ Connection Successful",
        "color: #00ff95; font-weight: bold",
      );
    ...
    this.socket.onclose = (event) => {
      console.warn(
        `[WS DEBUG] ⚠️ Connection Closed (Code: ${event.code}, Reason: ${event.reason})`,
      );
      this.socket = null;
  ...
  private attemptReconnect() {
    ...
    console.log(
      `WS: Reconnecting in 5s (Attempt ${this.reconnectAttempts})...`,
    );
  ...
  private handleMessage(message: any) {
    ...
    switch (event) {
      ...
      case "error":
        const errorDetail =
          message.error?.detail || message.data?.message || "An error occurred";
...
if (typeof window !== "undefined") {
  (window as any).simulateWS = (event: string, data: any) =>
    webSocketService.simulate(event, data);
}
