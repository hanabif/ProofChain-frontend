...
export const getLicenses = async (): Promise<License[]> => {
  try {
    const response = await api.get<PaginatedResponse<License>>(
      "/api/assets/licenses/me",
    );
...
export const createLicense = async (
  license: Omit<License, "id" | "assetCount" | "requestCount">,
): Promise<License> => {
...
export const updateLicense = async (
  ...
  const response = await api.patch<License>(
    `/api/assets/licenses/${id}`,
    updates,
  );
  return response.data;
};
